...
6. **Prepare boundary tets**: separate multiple boundary faces into different tets.
7. **Optimize** (optional): `numOptimizationIterations + numAdaptiveIterations` steps. Each does tet smoothing, then edge smoothing, then project (if enabled). Size-field split runs only on the adaptive steps, after the opt-only phase.
...
| `sizeFieldSmoothingIterations` | Jacobi averages of `h_max` on the input mesh. Default `3` |
...
See `TetrahedralizerParams` in `include/tetrahedralizer/Tetrahedralizer.h`. The optimize loop runs `numOptimizationIterations + numAdaptiveIterations` steps; both `0` skips it (no smooth, project, or split). Adaptive length parameters are in voxel spacings; the pipeline multiplies by `voxelSpacing` before sampling the size field.
...
- Interactive viewer: load OBJ, run tetrahedralization, inspect the tet mesh (clip planes, voxel size, opt/adaptive iters, project/optimize options); left-click the input mesh to set the orbit point
...
`createFiveTets` reads those eight ids. Unconnected faces never share nodes, so the tet mesh stays disconnected there; connected faces share all four corners and the alternating five-tet pattern still matches diagonals.

### Subdivision pipeline

Host-baked 64×diagBits child templates provide conforming subdivisions for every combination of subdivided tet edges. Used by `Tetrahedralizer::subdivide(maxEdgeLength)` (uniform threshold) and by adaptive remesh (per-node `h_max`; an edge splits when longer than `min(h_i, h_j)`):

1. **Subdivision vertices**: unique marked tet edges → append midpoint nodes.
...
Enabled when `numOptimizationIterations + numAdaptiveIterations > 0`. Runs after neighbors are built.
...
**Optimization** (`numOptimizationIterations + numAdaptiveIterations` times): tet smoothing if `volumeContraction > 0`, then edge smoothing if `edgeContraction > 0`, then project if enabled. On steps after the first `numOptimizationIterations`, a size-field split runs first (`runAdaptiveSplit`: sample, one split pass, rebuild neighbors, `separateBoundaryFaces`). Tet smoothing shape-matches to a regular tet. With projection on, surface nodes keep only the tangential part of the tet-smooth correction (`corr -= n * dot(corr, n)`). Edge smoothing with `useNormals` on contracts every edge then strips the normal component on surface nodes; with it off, mixed interior/surface edges move only the interior node. Both counts `0` skips the loop; split and projection do not run.

### Adaptive mesh

Goal: a spatially varying surface edge length so split (and later collapse) capture input curvature.

The size field is computed on the input mesh (`computeSurfaceSizeField`) and shown in the viewer (`Show size field`). Tet nodes do not own the field; they sample it. When `numAdaptiveIterations > 0`, `create` uploads the field, then `runOptimization` runs:

1. `numOptimizationIterations` opt-only steps (smooth, project) so the voxel mesh can settle onto the surface. Splitting earlier, on the stair-stepped grid, projects poorly.
2. `numAdaptiveIterations` steps that start with one size-field split, then the same smooth/project. One pass per step, not until idle.

`minEdgeLength` / `maxEdgeLength` / `geometricError` are voxel-relative clamps and ε; they are ignored when `numAdaptiveIterations` is `0`. Both iteration counts `0` skips the loop.

Each adaptive split (`runAdaptiveSplit` in `AdaptiveRemesh.cu`):

1. Classify surface nodes from boundary-face normals (interior normals stay zero).
2. Closest-point sample the input BVH: surface nodes take `min(h)` of the closest triangle's three vertices; interior nodes get the global ceiling `maxEdgeLength * voxelSpacing`.
3. Split unique tet edges longer than `min(h_i, h_j)`.
4. If anything split: rebuild neighbors and `separateBoundaryFaces`.

New midpoints are chord midpoints (off the input) until that same step's project snaps them.

#### Size field on the input mesh

`computeSurfaceSizeField` (`src/SizeField.cpp`) stores one scalar per **input-mesh vertex** (parallel to `positions`): the local maximum surface edge length `h_max`. CPU only. A tet edge uses the **minimum** of its endpoint samples (or the minimum of a triangle's three vertices) so the stricter vertex wins.
...
Split is implemented as above. Collapse from the size field is not. When it is added, hysteresis (`h_min ≈ 0.4–0.5` of local `h_max`) should keep split and collapse from fighting:

- Surface–surface collapse → midpoint
- Mixed interior/surface → keep the surface node
- Rebuild neighbors and `separateBoundaryFaces` after topology changes
- Project once per step after topology has settled, not after every collapse kernel

Interior tet edges keep the large `h_max` cap; curvature does not apply there.
...
| `src/tetrahedralizers/AdaptiveRemesh.cu` | Size-field sample and one split pass per adaptive step |
...
| `src/tetrahedralizers/Optimize.cu` | Optimize loop: optional adaptive split, shape-matching smooth, edge smooth, project |
