...
bool ImGui_ImplGLFW_Init(GLFWwindow* window, bool install_callbacks)
{
    ...
    io.RenderDrawListsFn = ImGui_ImplGLFW_RenderDrawData;
...
void ImGui_ImplGLFW_NewFrame()
{
    ...
    if (glfwGetWindowAttrib(g_Window, GLFW_FOCUSED))
    {
        ...
        io.MousePos = ImVec2((float)mouse_x, (float)mouse_y); // Mouse position, in pixels (set to -1,-1 if no mouse /
    ...
    for (int i = 0; i < 3; i++)
    {
        io.MouseDown[i] = g_MousePressed[i] || glfwGetMouseButton(g_Window, i) != 0; // If a mouse press event came,
