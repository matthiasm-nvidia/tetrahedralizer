...
/*
 ...
 - FREQUENTLY ASKED QUESTIONS (FAQ), TIPS
   ...
   - How can I have multiple widgets with the same label? Can I have widget without a label? (Yes). A primer on
 labels/IDs.
 ...
 - Efficient runtime and memory consumption (NB- we do allocate when "growing" content - creating a window / opening a
 tree node for the first time, etc. - but a typical frame won't allocate anything)
 ...
 - Your code creates the UI, if your code doesn't run the UI is gone! == very dynamic UI, no construction/destructions
 steps, less data retention on your side, no state duplication, less sync, less bugs.
 ...
 - Add the Dear ImGui source files to your projects, using your preferred build system. It is recommended you build the
 .cpp files as part of your project and not as a library.
 - You can later customize the imconfig.h file to tweak some compilation time behavior, such as integrating imgui types
 with your own maths types.
 - See examples/ folder for standalone sample applications. To understand the integration process, you can read
 examples/opengl2_example/ because it is short, then switch to the one more appropriate to your use case.
 ...
 - Init: retrieve the ImGuiIO structure with ImGui::GetIO() and fill the fields marked 'Settings': at minimum you need
 to set io.DisplaySize (application resolution). Later on you will fill your keyboard mapping, clipboard handlers, and
 other advanced features but for a basic integration you don't need to worry about it all.
 - Init: call io.Fonts->GetTexDataAsRGBA32(...), it will build the font atlas texture, then load the texture pixels into
 graphics memory.
 - Every frame:
    - In your main loop as early a possible, fill the IO fields marked 'Input' (e.g. mouse position, buttons, keyboard
 info, etc.)
    - Call ImGui::NewFrame() to begin the frame
    - You can use any ImGui function you want between NewFrame() and Render()
    - Call ImGui::Render() as late as you can to end the frame and finalize render data. it will call your
 io.RenderDrawListFn handler. (if you don't need to render, you still need to call Render() and ignore the callback, or
 call EndFrame() instead. if you call neither some aspects of windows focusing/moving will appear broken.)
 - All rendering information are stored into command-lists until ImGui::Render() is called.
 - Dear ImGui never touches or knows about your GPU state. the only function that knows about GPU is the
 RenderDrawListFn handler that you provide.
 - Effectively it means you can create widgets at any time in your code, regardless of considerations of being in
 "update" vs "render" phases of your own application.
 ...
 - A minimal application skeleton may be:
     ...
     io.RenderDrawListsFn = MyRenderFunction;  // Setup a render function, or set to NULL and call GetDrawData() after
 Render() to access the render data.
     ...
     // TODO: Store your texture pointer/identifier (whatever your engine uses) in 'io.Fonts->TexID'. This will be
 passed back to your via the renderer. io.Fonts->TexID = (void*)texture;

     // Application main loop
     while (true)
     {
        // Setup low-level inputs (e.g. on Win32, GetKeyboardState(), or write to those fields from your Windows message
 loop handlers, etc.) ImGuiIO& io = ImGui::GetIO(); io.DeltaTime = 1.0f/60.0f; io.MousePos = mouse_pos; io.MouseDown[0]
 = mouse_button_0; io.MouseDown[1] = mouse_button_1;
 ...
 multiply by vertex color. for (int n = 0; n < draw_data->CmdListsCount; n++)
       {
          ...
          for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
          {
             ...
             else
             {
                 ...
                 MyEngineScissor((int)pcmd->ClipRect.x, (int)pcmd->ClipRect.y, (int)(pcmd->ClipRect.z -
 pcmd->ClipRect.x), (int)(pcmd->ClipRect.w - pcmd->ClipRect.y)); MyEngineDrawIndexedTriangles(pcmd->ElemCount,
 ...
 - When calling NewFrame(), the 'io.WantCaptureMouse'/'io.WantCaptureKeyboard'/'io.WantTextInput' flags are updated.
   They tell you if ImGui intends to use your inputs. So for example, if 'io.WantCaptureMouse' is set you would
 typically want to hide mouse inputs from the rest of your application. Read the FAQ below for more information about
 those flags.
 ...
 Here is a change-log of API breaking changes, if you are using one of the functions listed, expect to have to fix some
 code. Also read releases logs https://github.com/ocornut/imgui/releases for more details.
 ...
 theme it will not affect you. However if your TitleBg/TitleBgActive alpha was <1.0f you need to tweak your custom theme
 to readjust for the fact that we don't draw a WindowBg background behind the title bar. This helper function will
 convert an old TitleBg/TitleBgActive color into a new one with the same visual output, given the OLD color and the OLD
 WindowBg color. ImVec4 ConvertTitleBgCol(const ImVec4& win_bg_col, const ImVec4& title_bg_col)
                           {
 ...
 intersect_with_current_clip_rect=false). Note that higher-level ImGui::PushClipRect() is preferable because it will
 clip at logic/widget level, whereas ImDrawList::PushClipRect() only affect your renderer.
 ...
 loader. (1.30) - removed GetDefaultFontData(). uses io.Fonts->GetTextureData*() API to retrieve uncompressed pixels.
                       ...
                       you now have much more flexibility to load multiple TTF fonts and manage the texture buffer for
 ...
 See http://www.patreon.com/imgui

 Q: What is ImTextureID and how do I display an image?
 A: ImTextureID is a void* used to pass renderer-agnostic texture references around until it hits your render function.
    Dear ImGui knows nothing about what those bits represent, it just passes them around. It is up to you to decide what
 you want the void* to carry! It could be an identifier to your OpenGL texture (cast GLuint to void*), a pointer to your
 custom engine material (cast MyMaterial* to void*), etc. At the end of the chain, your renderer takes this void* to
 cast it back into whatever it needs to select a current texture to render. Refer to examples applications, where each
 renderer (in a imgui_impl_xxxx.cpp file) is treating ImTextureID as a different thing. (c++ tip: OpenGL uses integers
 to identify textures. You can safely store an integer into a void*, just cast it to void*, don't take it's address!) To
 display a custom image/texture within an ImGui window, you may use ImGui::Image(), ImGui::ImageButton(),
 ImDrawList::AddImage() functions. Dear ImGui will generate the geometry and draw calls using the ImTextureID that you
 passed and which your renderer can use. It is your responsibility to get textures uploaded to your GPU.
 ...
 A: Most likely you are mishandling the clipping rectangles in your render function. Rectangles provided by ImGui are
 ...
 A: Yes. A primer on the use of labels/IDs in Dear ImGui..
   ...
   - Interactive widgets require state to be carried over multiple frames (most typically Dear ImGui often needs to
 remember what is the "active" widget). to do so they need a unique ID. unique ID are typically derived from a string
 label, an integer index or a pointer.

       Button("OK");        // Label = "OK",     ID = hash of "OK"
       Button("Cancel");    // Label = "Cancel", ID = hash of "Cancel"

   - ID are uniquely scoped within windows, tree nodes, etc. so no conflict can happen if you have two buttons called
 "OK" in two different windows or in two different locations of a tree.
   ...
   - When passing a label you can optionally specify extra unique ID information within string itself. This helps
 solving the simpler collision cases. use "##" to pass a complement to the ID that won't be visible to the end-user:
   ...
   - Occasionally/rarely you might want change a label while preserving a constant ID. This allows you to animate
 labels. For example you may want to include varying information in a window title bar (and windows are uniquely
 identified by their ID.. obviously) Use "###" to pass a label that isn't part of ID:
   ...
   - Use PushID() / PopID() to create scopes and avoid ID conflicts within the same Window.
     ...
     You can push a pointer, a string or an integer value. Remember that ID are formed from the concatenation of
 everything in the ID stack!
   ...
   - When working with trees, ID are used to preserve the open/close state of each tree node.
     Depending on your use cases you may want to use strings, indices or pointers as ID.
      e.g. when displaying a single object that may change over time (1-1 relationship), using a static string as ID
 will preserve your node open/closed state when the targeted object change. e.g. when displaying a list of objects,
 using indices or pointers as ID will preserve the node open/closed state differently. experiment and see what makes
 more sense!
 ...
 A: You can read the 'io.WantCaptureMouse'/'io.WantCaptureKeyboard'/'ioWantTextInput' flags from the ImGuiIO structure.
    - When 'io.WantCaptureMouse' or 'io.WantCaptureKeyboard' flags are set you may want to discard/hide the inputs from
 the rest of your application.
    - When 'io.WantTextInput' is set to may want to notify your OS to popup an on-screen keyboard, if available (e.g. on
 a mobile phone, or console without a keyboard). Preferably read the flags after calling ImGui::NewFrame() to avoid them
 lagging by one frame. But reading those flags before calling NewFrame() is also generally ok, as the bool toggles
 fairly rarely and you don't generally expect to interact with either Dear ImGui or your application during the same
 frame when that transition occurs. Dear ImGui is tracking dragging and widget activity that may occur outside the
 boundary of a window, so 'io.WantCaptureMouse' is more accurate and correct than checking if a window is hovered.
    (Advanced note: text input releases focus on Return 'KeyDown', so the following Return 'KeyUp' event that your
 application receive will typically have 'io.WantCaptureKeyboard=false'. Depending on your application logic it may or
 not be inconvenient. You might want to track which key-downs were for Dear ImGui (e.g. with an array of bool) and
 filter out the corresponding key-ups.)

 Q: How can I load a different font than the default? (default is an embedded version of ProggyClean.ttf, rendered at
 size 13) A: Use the font atlas to load the TTF/OTF file you want:
 ...
 A: The most convenient and practical way is to merge an icon font such as FontAwesome inside you main font. Then you
 can refer to icons within your strings. Read 'How can I load multiple fonts?' and the file 'extra_fonts/README.txt' for
 instructions.
 ...
 unique characters submitted) io.Fonts->AddFontFromFileTTF("myfontfile.ttf", size_in_pixels, NULL, ranges.Data);
    ...
    All your strings needs to use UTF-8 encoding. In C++11 you can encode a string literal in UTF-8 by using the
 u8"hello" syntax. Specifying literal in your source code using a local code page (such as CP-923 for Japanese or
 CP-1251 for Cyrillic) will NOT work! Otherwise you can convert yourself to UTF-8 or load text data from file already
 saved as UTF-8.

    Text input: it is up to your application to pass the right character code to io.AddInputCharacter(). The
 applications in examples/ are doing that. For languages using IME, on Windows you can copy the Hwnd of your application
 to io.ImeWindowHandle. The default implementation of io.ImeSetInputScreenPosFn() on Windows will set your IME position
 correctly.

 Q: How can I preserve my Dear ImGui context across reloading a DLL? (loss of the global/static variables)
 A: Create your own context 'ctx = CreateContext()' + 'SetCurrentContext(ctx)' and your own font atlas
 'ctx->GetIO().Fonts = new ImFontAtlas()' so you don't rely on the default globals.

 Q: How can I use the drawing facilities without an ImGui window? (using ImDrawList API)
 A: The easiest way is to create a dummy window. Call Begin() with
 NoTitleBar|NoResize|NoMove|NoScrollbar|NoSavedSettings|NoInputs flag, zero background alpha, then retrieve the
 ImDrawList* via GetWindowDrawList() and draw to it in any way you like. You can also perfectly create a standalone
 ImDrawList instance _but_ you need ImGui to be initialized because ImDrawList pulls from ImGui data to retrieve the
 coordinates of the white pixel.

 - tip: the construct 'IMGUI_ONCE_UPON_A_FRAME { ... }' will run the block of code only once a frame. You can use it to
 quickly add custom UI in the middle of a deep nested inner loop in your code.
...
#ifdef __clang__
#    pragma clang diagnostic ignored "-Wunknown-pragmas" // warning : unknown warning group '-Wformat-pedantic *' // not
                                                         // all warnings are known by all clang versions.. so ignoring
...
#    pragma clang diagnostic ignored "-Wfloat-equal" // warning : comparing floating point with == or != is unsafe   //
                                                     ...
                                                     // ok.
#    pragma clang diagnostic ignored "-Wformat-nonliteral" // warning : format string is not a string literal // passing
                                                           // non-literal to vsnformat(). yes, user passing incorrect
                                                           // format strings can crash the code.
#    pragma clang diagnostic ignored "-Wexit-time-destructors" // warning : declaration requires an exit-time destructor
                                                               // // exit-time destruction order is undefined. if
                                                               // MemFree() leads to users code that has been disabled
                                                               // before exit it might cause problems. ImGui coding
...
#    pragma GCC diagnostic ignored "-Wformat" // warning: format '%p' expects argument of type 'void*', but argument 6
...
// ImGui::SetCurrentContext() If you are hot-reloading this code in a DLL you will lose the static/global variables.
// Create your own context+font atlas instead of relying on those default (see FAQ entry "How can I preserve my ImGui
// context across reloading a DLL?"). ImGui is currently not thread-safe because of this variable. If you want
// thread-safety to allow N threads to access N different contexts, you might work around it by:
...
// - or: Changing this variable to be TLS. You may #define GImGui in imconfig.h for further custom hackery. Future
...
ImGuiStyle::ImGuiStyle()
{
    ...
    ChildWindowRounding = 0.0f; // Radius of child window corners rounding. Set to 0.0f to have rectangular child
                                // windows
    ...
    DisplayWindowPadding = ImVec2(22, 22); // Window positions are clamped to be visible within the display area by at
                                           // least this amount. Only covers regular windows.
    DisplaySafeAreaPadding = ImVec2(4, 4); // If you cannot see the edge of your screen (e.g. on a TV) increase the safe
                                           // area padding. Covers popups/tooltips as well regular windows.
    AntiAliasedLines = true; // Enable anti-aliasing on lines/borders. Disable if you are really short on CPU/GPU.
    AntiAliasedShapes = true; // Enable anti-aliasing on filled shapes (rounded rectangles, circles, etc.)
    CurveTessellationTol = 1.25f; // Tessellation tolerance. Decrease for highly tessellated curves (higher quality,
...
void ImGui::StyleColorsClassic(ImGuiStyle* dst)
{
    ...
    colors[ImGuiCol_FrameBg] = ImVec4(0.80f, 0.80f, 0.80f, 0.30f); // Background of checkbox, radio button, plot,
                                                                   // slider, text input
...
// function. Scaling the fonts is done separately and is up to you. Tips: if you need to change your scale multiple
// times, prefer calling this on a freshly initialized ImGuiStyle structure rather than scaling multiple times (because
// floating point multiplications are lossy).
...
// MSVC version appears to return -1 on overflow, whereas glibc appears to return total count (which may be >=
// buf_size). Ideally we would test for only one of those limits at runtime depending on the behavior the vsnprintf(),
// but trying to deduct it at compile time sounds like a pandora can of worm.
...
void ImGuiTextBuffer::appendv(const char* fmt, va_list args)
{
    ...
    int len = vsnprintf(NULL, 0, fmt, args); // FIXME-OPT: could do a first pass write attempt, likely successful on
                                             // first pass.
    if (len <= 0)
        return;
...
float ImGuiSimpleColumns::DeclColumns(float w0, float w1, float w2) // not using va_arg because they promote float to
                                                                    // double
{
    NextWidth = 0.0f;
...
static void SetCursorPosYAndSetupDummyPrevLine(float pos_y, float line_height)
{
    ...
    // the same issue. Consider moving within SetCursorXXX functions?
    ImGui::SetCursorPosY(pos_y);
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    window->DC.CursorPosPrevLine.y = window->DC.CursorPos.y - line_height; // Setting those fields so that
                                                                           // SetScrollHere() can properly function
                                                                           // after the end of our clipper usage.
    window->DC.PrevLineHeight = (line_height - GImGui->Style.ItemSpacing.y); // If we end up needing more accurate data
                                                                             // (to e.g. use SameLine) we may as well
                                                                             // make the clipper have a fourth step to
                                                                             // let user process and display the last
                                                                             // item in their list.
...
void ImGuiListClipper::Begin(int count, float items_height)
{
    ...
    if (ItemsHeight > 0.0f)
    {
        ImGui::CalcListClipping(ItemsCount, ItemsHeight, &DisplayStart, &DisplayEnd); // calculate how many to
                                                                                      // clip/display
        if (DisplayStart > 0)
...
bool ImGuiListClipper::Step()
{
    ...
    if (StepNo == 3) // Step 3: the clipper validate that we have reached the expected Y position (corresponding to
...
bool ImGui::FocusableItemRegister(ImGuiWindow* window, ImGuiID id, bool tab_stop)
{
    ...
    if (tab_stop && (g.ActiveId == id) && window->FocusIdxAllRequestNext == INT_MAX &&
        ...
        window->FocusIdxTabRequestNext =
            window->FocusIdxTabCounter + (g.IO.KeyShift ? (allow_keyboard_focus ? -1 : 0) : +1); // Modulo on index will
                                                                                                 // be applied at the
                                                                                                 // end of frame once
                                                                                                 // we've got the total
                                                                                                 // counter of items.
...
void ImGui::NewFrame()
{
    ...
    // Check user data
    IM_ASSERT(g.IO.DeltaTime >= 0.0f); // Need a positive DeltaTime (zero is tolerated but will cause some timing
                                       // issues)
    IM_ASSERT(g.IO.DisplaySize.x >= 0.0f && g.IO.DisplaySize.y >= 0.0f);
    IM_ASSERT(g.IO.Fonts->Fonts.Size > 0); // Font Atlas not created. Did you call io.Fonts->GetTexDataAsRGBA32 /
                                           // GetTexDataAsAlpha8 ?
    IM_ASSERT(g.IO.Fonts->Fonts[0]->IsLoaded()); // Font Atlas not created. Did you call io.Fonts->GetTexDataAsRGBA32 /
                                                 // GetTexDataAsAlpha8 ?
    IM_ASSERT(g.Style.CurveTessellationTol > 0.0f); // Invalid style setting
    IM_ASSERT(g.Style.Alpha >= 0.0f && g.Style.Alpha <= 1.0f); // Invalid style setting. Alpha cannot be negative
                                                               // (allows us to avoid a few clamps in color
                                                               // computations)
    ...
    // If mouse just appeared or disappeared (usually denoted by -FLT_MAX component, but in reality we test for
    // -256000.0f) we cancel out movement in MouseDelta
    ...
    // Handle user moving window with mouse (at the beginning of the frame to avoid input lag or sheering). Only valid
    // for root windows.
    ...
    // When clicking outside of a window we assume the click is owned by the application and won't request capture. We
    ...
    g.IO.WantCaptureKeyboard =
        (g.WantCaptureKeyboardNextFrame != -1) ? (g.WantCaptureKeyboardNextFrame != 0) : (g.ActiveId != 0);
...
void ImGui::Shutdown()
{
    ...
    if (g.IO.Fonts) // Testing for NULL to allow user to NULLify in case of running Shutdown() on multiple contexts. Bit
                    // hacky.
        g.IO.Fonts->Clear();
...
static void SaveIniSettingsToDisk(const char* ini_filename)
{
    ...
    for (int i = 0; i != g.Windows.Size; i++)
    {
        ...
        if (!settings) // This will only return NULL in the rare instance where the window was first created with
                       // ImGuiWindowFlags_NoSavedSettings then had the flag disabled later on. We don't bind settings
                       // in this case (bug #1000).
            continue;
    ...
    for (int i = 0; i != g.Settings.Size; i++)
    {
        ...
        if (const char* p = strstr(name, "###")) // Skip to the "###" marker if any. We don't skip past to match the
                                                 // behavior of GetID()
            name = p;
        fprintf(f, "[%s]\n", name);
...
static void AddDrawListToRenderList(ImVector<ImDrawList*>& out_render_list, ImDrawList* draw_list)
{
    ...
    // Check that draw_list doesn't use more vertices than indexable in a single draw call (default ImDrawIdx = unsigned
    // short = 2 bytes = 64K vertices per window) If this assert triggers because you are drawing lots of stuff
    // manually, you can: A) Add '#define ImDrawIdx unsigned int' in imconfig.h to set the index size to 4 bytes. You'll
    // need to handle the 4-bytes indices to your renderer.
    ...
    //    GL_UNSIGNED_INT, idx_buffer_offset);' Your own engine or render API may use different parameters or function
    //    calls to specify index sizes. 2 and 4 bytes indices are generally supported by most API.
    ...
    IM_ASSERT(((ImU64)draw_list->_VtxCurrentIdx >> (sizeof(ImDrawIdx) * 8)) == 0); // Too many vertices in same
                                                                                   // ImDrawList. See comment above.
...
void ImGui::EndFrame()
{
    ...
    IM_ASSERT(g.FrameCountEnded != g.FrameCount); // ImGui::EndFrame() called multiple times, or forgot to call
                                                  // ImGui::NewFrame() again
...
void ImGui::RenderCheckMark(ImVec2 pos, ImU32 col)
{
    ...
    float by = pos.y - 1.0f + (float)(int)(g.Font->Ascent * (g.FontSize / g.Font->FontSize) + 0.5f) +
...
// SetWindowPos() and not SetNextWindowPos() will have that rectangle lagging by a frame at the time FindHoveredWindow()
// is called, aka before the next Begin(). Moving window thankfully isn't affected.
static ImGuiWindow* FindHoveredWindow(ImVec2 pos, bool excluding_childs)
{
    ImGuiContext& g = *GImGui;
    for (int i = g.Windows.Size - 1; i >= 0; i--)
...
// Note that imgui doesn't know the semantic of each entry of io.KeyDown[]. Use your own indices/enums according to how
// your backend/engine stored them into KeyDown[]!
...
ImVec2 ImGui::GetMouseDragDelta(int button, float lock_threshold)
{
    ...
    if (g.IO.MouseDown[button])
        if (g.IO.MouseDragMaxDistanceSqr[button] >= lock_threshold * lock_threshold)
            return g.IO.MousePos - g.IO.MouseClickedPos[button]; // Assume we can only get active with left-mouse button
                                                                 // (at the moment).
...
void ImGui::OpenPopupEx(ImGuiID id, bool reopen_existing)
{
    ...
    ImGuiPopupRef popup_ref = ImGuiPopupRef(id, window, window->GetID("##menus"), g.IO.MousePos); // Tagged as new ref
                                                                                                  // because constructor
                                                                                                  // sets Window to NULL
                                                                                                  // (we are passing the
                                                                                                  // ParentWindow info
    ...
    if (g.NavWindow)
    {
        for (n = 0; n < g.OpenPopupStack.Size; n++)
        {
            ...
            for (int m = n; m < g.OpenPopupStack.Size && !has_focus; m++)
                has_focus =
                    (g.OpenPopupStack[m].Window && g.OpenPopupStack[m].Window->RootWindow == g.NavWindow->RootWindow);
            if (!has_focus)
                break;
        }
    }
    if (n < g.OpenPopupStack.Size) // This test is not required but it allows to set a useful breakpoint on the line
                                   // below
...
bool ImGui::BeginPopupEx(ImGuiID id, ImGuiWindowFlags extra_flags)
{
    ...
    if (flags & ImGuiWindowFlags_ChildMenu)
        ImFormatString(name, IM_ARRAYSIZE(name), "##menu_%d", g.CurrentPopupStack.Size); // Recycle windows based on
                                                                                         // depth
    else
...
// 1. If you have many possible popups (for different "instances" of a same widget, or for wholly different widgets),
// you may be better off handling
//    this yourself so you can store data relative to the widget that opened the popup instead of choosing different
//    popup identifiers.
// 2. If you want right-clicking on the same item to reopen the popup at new location, use the same code replacing
// IsItemHovered() with IsItemRectHovered()
//    and passing true to the OpenPopupEx().
//    Because: hovering an item in a window below the popup won't normally trigger is hovering behavior/coloring. The
//    pattern of ignoring the fact that the item can be interacted with (because it is blocked by the active popup) may
//    useful in some situation when e.g. large canvas as one item, content of menu driven by click position.
...
static bool BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
    ...
    if (size.x <= 0.0f)
        size.x = ImMax(content_avail.x, 4.0f) - fabsf(size.x); // Arbitrary minimum zero-ish child size of 4.0f (0.0f
...
void ImGui::EndChild()
{
    ...
    {
        // When using auto-filling child window, we don't provide full width/height to ItemSize so that it doesn't feed
        // back into automatic size-fitting.
        ImVec2 sz = GetWindowSize();
        if (window->AutoFitChildAxises & 0x01) // Arbitrary minimum zero-ish child size of 4.0f causes less trouble than
                                               // a 0.0f
...
bool ImGui::BeginChildFrame(ImGuiID id, const ImVec2& size, ImGuiWindowFlags extra_flags)
{
    ...
    return BeginChild(id, size, (g.CurrentWindow->Flags & ImGuiWindowFlags_ShowBorders) ? true : false,
...
static ImVec2 CalcSizeAutoFit(ImGuiWindow* window)
{
    ...
    {
        // Handling case of auto fit window not fitting on the screen (on either axis): we are growing the size on the
        // other axis to compensate for expected scrollbar. FIXME: Might turn bigger than DisplaySize-WindowPadding.
        ...
        if (size_auto_fit_after_constraint.x < window->SizeContents.x && !(flags & ImGuiWindowFlags_NoScrollbar) &&
...
static ImVec2 CalcNextScrollFromScrollTargetAndClamp(ImGuiWindow* window)
{
    ...
    if (window->ScrollTarget.y < FLT_MAX)
        scroll.y = window->ScrollTarget.y - (1.0f - cr_y) * (window->TitleBarHeight() + window->MenuBarHeight()) -
                   cr_y * (window->SizeFull.y - window->ScrollbarSizes.y);
    scroll = ImMax(scroll, ImVec2(0.0f, 0.0f));
    if (!window->Collapsed && !window->SkipItems)
    {
        scroll.x = ImMin(scroll.x, ImMax(0.0f, window->SizeContents.x -
                                                   (window->SizeFull.x - window->ScrollbarSizes.x))); // ==
                                                                                                      // GetScrollMaxX
                                                                                                      // for
                                                                                                      // that
                                                                                                      // window
        scroll.y = ImMin(scroll.y, ImMax(0.0f, window->SizeContents.y -
...
// - 'size_on_first_use' for a regular window denote the initial size for first-time creation (no saved data) and isn't
// that useful. Use SetNextWindowSize() prior to calling Begin() for more flexible window manipulation.
...
// - Passing non-zero 'size' is roughly equivalent to calling SetNextWindowSize(size, ImGuiCond_FirstUseEver) prior to
// calling Begin().
...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags)
{
    ...
    for (root_non_popup_idx = root_idx; root_non_popup_idx > 0; root_non_popup_idx--)
        if (!(g.CurrentWindowStack[root_non_popup_idx]->Flags & (ImGuiWindowFlags_ChildWindow | ImGuiWindowFlags_Popup)) ||
            (g.CurrentWindowStack[root_non_popup_idx]->Flags & ImGuiWindowFlags_Modal))
            break;
    window->ParentWindow = parent_window;
    window->RootWindow = g.CurrentWindowStack[root_idx];
    window->RootNonPopupWindow = g.CurrentWindowStack[root_non_popup_idx]; // Used to display TitleBgActive color and
                                                                           // for selecting which window to use for
                                                                           // NavWindowing
    ...
    if (first_begin_of_the_frame)
    {
        ...
        if ((flags & ImGuiWindowFlags_ChildWindow) && !(flags & ImGuiWindowFlags_Popup))
        {
            ...
            window->Size = window->SizeFull = size_on_first_use; // NB: argument name 'size_on_first_use' misleading
                                                                 // here, it's really just 'size' as provided by user
        ...
        else if (flags & ImGuiWindowFlags_ChildMenu)
        {
            ...
            // FindBestPopupWindowPos() function will move the new menu outside the parent bounds. This is how we end up
            // with child menus appearing (most-commonly) on the right of the parent menu.
            IM_ASSERT(window_pos_set_by_api);
            float horizontal_overlap = style.ItemSpacing.x; // We want some overlap to convey the relative depth of each
                                                            // popup (currently the amount of overlap it is hard-coded
                                                            // to style.ItemSpacing.x, may need to introduce another
                                                            // style value).
        ...
        if ((flags & ImGuiWindowFlags_Tooltip) != 0 && !window_pos_set_by_api)
        {
            ...
            ImRect rect_to_avoid(ref_pos.x - 16, ref_pos.y - 8, ref_pos.x + 24, ref_pos.y + 24); // FIXME: Completely
                                                                                                 ...
                                                                                                 // hit-point instead?
            window->PosFloat =
                FindBestPopupWindowPos(ref_pos, window->Size, &window->AutoPosLastDirection, rect_to_avoid);
            if (window->AutoPosLastDirection == -1)
                window->PosFloat = ref_pos + ImVec2(2, 2); // If there's not enough room, for tooltip we prefer avoiding
        ...
        if (!(flags & ImGuiWindowFlags_ChildWindow) && !(flags & ImGuiWindowFlags_Tooltip))
        {
            if (!window_pos_set_by_api && window->AutoFitFramesX <= 0 && window->AutoFitFramesY <= 0 &&
                g.IO.DisplaySize.x > 0.0f && g.IO.DisplaySize.y > 0.0f) // Ignore zero-sized display explicitly to avoid
        ...
        if (window->Collapsed)
        {
            // Title bar only
            RenderFrame(
                title_bar_rect.Min, title_bar_rect.Max, GetColorU32(ImGuiCol_TitleBgCollapsed), true, window_rounding);
        }
        else
        {
            ImU32 resize_col = 0;
            ...
            {
                // Manual resize
                // Using the FlattenChilds button flag, we make the resize button accessible even if we are hovering
                // over a child window
            ...
            if (window->ScrollbarX && !window->ScrollbarY)
                window->ScrollbarY =
                    (window->SizeContents.y > window->Size.y + style.ItemSpacing.y - style.ScrollbarSize) &&
                    !(flags & ImGuiWindowFlags_NoScrollbar);
            ...
            ImVec4 bg_color = style.Colors[bg_color_idx]; // We don't use GetColorU32() because bg_alpha is assigned
            ...
            if (bg_color.w > 0.0f)
                window->DrawList->AddRectFilled(
                    ...
                    (flags & ImGuiWindowFlags_NoTitleBar) ? ImGuiCorner_All : ImGuiCorner_BotLeft | ImGuiCorner_BotRight);
        ...
        // New windows appears in front (we need to do that AFTER setting DC.CursorStartPos so our initial navigation
    ...
    // Mark them as collapsed so commands are skipped earlier (we can't manually collapse because they have no title
    // bar).
    if (flags & ImGuiWindowFlags_ChildWindow)
    {
...
static void Scrollbar(ImGuiWindow* window, bool horizontal)
{
    ...
    IM_ASSERT(ImMax(win_size_contents_v, win_size_avail_v) > 0.0f); // Adding this assert to check if the
                                                                    // ImMax(XXX,1.0f) is still needed. PLEASE CONTACT
                                                                    // ME if this triggers.
...
static void SetWindowScrollY(ImGuiWindow* window, float new_scroll_y)
{
    window->DC.CursorMaxPos.y += window->Scroll.y; // SizeContents is generally computed based on CursorMaxPos which is
...
static void SetWindowPos(ImGuiWindow* window, const ImVec2& pos, ImGuiCond cond)
{
    ...
    window->PosFloat = pos;
    window->Pos = ImVec2((float)(int)window->PosFloat.x, (float)(int)window->PosFloat.y);
    window->DC.CursorPos += (window->Pos - old_pos); // As we happen to move the window while it is being appended to
                                                     // (which is a bad idea - will smear) let's at least offset the
                                                     // cursor
    window->DC.CursorMaxPos += (window->Pos - old_pos); // And more importantly we need to adjust this so size
...
void ImGui::SetScrollFromPosY(float pos_y, float center_y_ratio)
{
    ...
    if (center_y_ratio <= 0.0f && window->ScrollTarget.y <= window->WindowPadding.y) // Minor hack to make "scroll to
                                                                                     // top" take account of
                                                                                     // WindowPadding, else it would
...
void ImGui::SetScrollHere(float center_y_ratio)
{
    ...
    target_y += (window->DC.PrevLineHeight * center_y_ratio) +
                (GImGui->Style.ItemSpacing.y * (center_y_ratio - 0.5f) * 2.0f); // Precisely aim above, in the middle or
                                                                                // below the last line.
...
{
    bool need_wrap = (GImGui->CurrentWindow->DC.TextWrapPos < 0.0f); // Keep existing wrap position is one ia already
                                                                     // set
    if (need_wrap)
...
void ImGui::TextUnformatted(const char* text, const char* text_end)
{
    ...
    if (text_end - text > 2000 && !wrap_enabled)
    {
        ...
        // word-wrapping is disabled. We also don't vertically center the text within the line full height, which is
        ...
        if (!(flags & ImGuiButtonFlags_NoKeyModifiers) || (!g.IO.KeyCtrl && !g.IO.KeyShift && !g.IO.KeyAlt))
        {
            ...
            if ((flags & ImGuiButtonFlags_PressedOnRelease) && g.IO.MouseReleased[0])
            {
                if (!((flags & ImGuiButtonFlags_Repeat) &&
                      g.IO.MouseDownDurationPrev[0] >= g.IO.KeyRepeatDelay)) // Repeat
                                                                             // mode
                                                                             // trumps
                                                                             // <on
                                                                             // release>
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if ((flags & ImGuiButtonFlags_AlignTextBaseLine) &&
        style.FramePadding.y < window->DC.CurrentLineTextBaseOffset) // Try to vertically align buttons that are
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    const float text_base_offset_y = ImMax(padding.y, window->DC.CurrentLineTextBaseOffset); // Latch before ItemSize
                                                                                             // changes it
    ...
    const float text_offset_x = (g.FontSize + (display_frame ? padding.x * 3 : padding.x * 2)); // Collapser arrow width
                                                                                                // + Spacing
...
// ImGuiTreeNodeFlags_CollapsingHeader | ImGuiTreeNodeFlags_NoTreePushOnOpen). You can remove the _NoTreePushOnOpen flag
...
void ImGui::BulletTextV(const char* fmt, va_list args)
{
    ...
    const float text_base_offset_y = ImMax(0.0f, window->DC.CurrentLineTextBaseOffset); // Latch before ItemSize changes
                                                                                        // it
    const float line_height =
        ImMax(ImMin(window->DC.CurrentLineHeight, g.FontSize + g.Style.FramePadding.y * 2), g.FontSize);
...
static inline void DataTypeFormatString(ImGuiDataType data_type, void* data_ptr, int decimal_precision, char* buf, int buf_size)
{
    ...
    {
        if (decimal_precision < 0)
            ImFormatString(buf, buf_size, "%f", *(float*)data_ptr); // Ideally we'd have a minimum decimal precision of
                                                                    // 1 to visually denote that it is a float, while
                                                                    // hiding non-significant digits?
        else
    ...
    const ImRect& aabb, const char* label, ImGuiDataType data_type, void* data_ptr, ImGuiID id, int decimal_precision)
{
    ...
    {
        IM_ASSERT(g.ActiveId == id); // InputText ID expected to match the Slider ID (else we'd need to store them both,
                           ...
                           ImGuiSliderFlags flags)
{
    ...
    // For logarithmic sliders that cross over sign boundary we want the exponential increase to be symmetric around
    // 0.0f
...
bool ImGui::DragBehavior(
    const ImRect& frame_bb, ImGuiID id, float* v, float v_speed, float v_min, float v_max, int decimal_precision, float power)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
                   ...
                   ImVec2 graph_size)
{
    ...
    if (values_count > 0)
    {
        ...
        float histogram_zero_line_t = (scale_min * scale_max < 0.0f) ? (-scale_min / (scale_max - scale_min)) :
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb(
        window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.y + style.FramePadding.y * 2,
                                                            label_size.y + style.FramePadding.y * 2)); // We want a
                                                                                                       // square shape
                                                                                                       // to we use Y
                                                                                                       // twice
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    const ImRect check_bb(
        window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.y + style.FramePadding.y * 2 - 1,
...
// NB: when active, hold on a privately held copy of the text (and apply back to 'buf'). So changing 'buf' while active
// has no effect.
                        ...
                        void* user_data)
{
    ...
    IM_ASSERT(!((flags & ImGuiInputTextFlags_CallbackHistory) && (flags & ImGuiInputTextFlags_Multiline))); // Can't use
                                                                                                            ...
                                                                                                            // keys)
    IM_ASSERT(
        !((flags & ImGuiInputTextFlags_CallbackCompletion) && (flags & ImGuiInputTextFlags_AllowTabInput))); // Can't
                                                                                                             // use
                                                                                                             // both
                                                                                                             // together
                                                                                                             // (they
    ...
    ImVec2 size = CalcItemSize(
        ...
        (is_multiline ? GetTextLineHeight() * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary default
                                                                                                   // of 8 lines high
                                                                                                   // for multi-line
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + size);
    const ImRect total_bb(
    ...
    const bool focus_requested = FocusableItemRegister(
        window, id,
        (flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_AllowTabInput)) == 0); // Using
                                                                                                      // completion
                                                                                                      // callback
                                                                                                      // disable
                                                                                                      // keyboard
    ...
    if (focus_requested || user_clicked || user_scrolled)
    {
        if (g.ActiveId != id)
        {
            ...
            edit_state.Text.resize(buf_size + 1); // wchar count <= UTF-8 count. we use +1 to make sure that .Data isn't
                                                  // NULL so it doesn't crash.
            edit_state.InitialText.resize(buf_size + 1); // UTF-8. we use +1 to make sure that .Data isn't NULL so it
                                                         // doesn't crash.
            ImStrncpy(edit_state.InitialText.Data, buf, edit_state.InitialText.Size);
            const char* buf_end = NULL;
            edit_state.CurLenW = ImTextStrFromUtf8(edit_state.Text.Data, edit_state.Text.Size, buf, NULL, &buf_end);
            edit_state.CurLenA = (int)(buf_end - buf); // We can't get the result from ImFormatString() above because it
    ...
    if (g.ActiveId == id)
    {
        ...
        const bool osx_double_click_selects_words = io.OSXBehaviors; // OS X style: Double click selects by word instead
        ...
        if (io.InputCharacters[0])
        {
            ...
            // We ignore CTRL inputs, but need to allow CTRL+ALT as some keyboards (e.g. German) use AltGR - which is
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const bool is_wordmove_key_down = io.OSXBehaviors ? io.KeyAlt : io.KeyCtrl; // OS X style: Text editing cursor
                                                                                    // movement using Alt instead of
                                                                                    // Ctrl
        const bool is_startend_key_down = io.OSXBehaviors && io.KeySuper && !io.KeyCtrl && !io.KeyAlt; // OS X style:
                                                                                                       // Line/Text
                                                                                                       // Start and End
                                                                                                       // using
                                                                                                       // Cmd+Arrows
    ...
    const ImVec4 clip_rect(
        frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + size.x, frame_bb.Min.y + size.y); // Not
                                                                                           // using
                                                                                           // frame_bb.Max
                                                                                           // because
                                                                                           // we have
        ...
        if (edit_state.StbState.select_start != edit_state.StbState.select_end)
        {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end;)
            {
                ...
                else
                {
                    ...
                    if (rect_size.x <= 0.0f)
                        rect_size.x = (float)(int)(g.Font->GetCharAdvance((unsigned short)' ') * 0.50f); // So we can
                                                                                                         // see selected
                                                                                                         // empty lines
                    ImRect rect(
    ...
    else
    {
        ...
        const char* buf_end = NULL;
        if (is_multiline)
            text_size = ImVec2(size.x, InputTextCalcTextLenAndLineCount(buf_display, &buf_end) * g.FontSize); // We
                                                                                                              // don't
                                                                                                              // need
                                                                                                              // width
                          ...
                          ImGuiInputTextFlags extra_flags)
{
    ...
    if (InputText("", buf, IM_ARRAYSIZE(buf), extra_flags)) // PushId(label) + "" gives us the expected ID from outside
                                                            // point of view
        value_changed =
    ...
    const char* label, float* v, float step, float step_fast, int decimal_precision, ImGuiInputTextFlags extra_flags)
{
    ...
    if (decimal_precision < 0)
        strcpy(display_format, "%f"); // Ideally we'd have a minimum decimal precision of 1 to visually denote that this
                                      // is a float, while hiding non-significant digits? %f doesn't have a minimum of 1
    else
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImVec2 popup_size)
{
    ...
    RenderCollapseTriangle(
        ImVec2(frame_bb.Max.x - arrow_size + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y), true);
                  ...
                  int height_in_items)
{
    ...
    if (height_in_items < 0)
        height_in_items = 7;
    float popup_height =
        (g.FontSize + style.ItemSpacing.y) * ImMin(items_count, height_in_items) + (style.FramePadding.y * 3);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if ((flags & ImGuiSelectableFlags_SpanAllColumns) && window->DC.ColumnsCount > 1) // FIXME-OPT: Avoid if vertically
                                                                                      // clipped.
        PopClipRect();
...
{
    // Size default to hold ~7 items. Fractional number of items helps seeing that we can scroll down/up without looking
    // at scrollbar. However we don't add +0.40f if items_count <= height_in_items. It is slightly dodgy, because it
    // means a dynamic list of items will make the widget resize occasionally when it crosses that size. I am expecting
    // that someone will come and complain about this behavior in a remote future, then we can advise on a better
    // solution.
    if (height_in_items < 0)
                    ...
                    int height_in_items)
{
    ...
    bool value_changed = false;
    ImGuiListClipper clipper(items_count, GetTextLineHeightWithSpacing()); // We know exactly our line height here so we
                                                                           // pass it as a minor optimization, but
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled)
{
    ...
    float w = window->MenuColumns.DeclColumns(label_size.x, shortcut_size.x, (float)(int)(g.FontSize * 1.20f)); // Feedback
                                                                                                                // for
                                                                                                                // next
                                                                                                                // frame
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        pressed = Selectable(label, menu_is_open,
                             ...
                             ImVec2(w, 0.0f));
        PopStyleVar();
        window->DC.CursorPos.x += (float)(int)(style.ItemSpacing.x * (-1.0f + 0.5f)); // -1 spacing to compensate the
                                                                                      // spacing added when Selectable()
                                                                                      // did a SameLine(). It would also
                                                                                      // work to call SameLine()
                                                                                      // ourselves after the
                                                                                      // PopStyleVar().
    }
    else
    {
        ...
        float w = window->MenuColumns.DeclColumns(label_size.x, 0.0f, (float)(int)(g.FontSize * 1.20f)); // Feedback to
                                                                                                         // next frame
        float extra_w = ImMax(0.0f, GetContentRegionAvail().x - w);
        pressed = Selectable(label, menu_is_open,
                             ImGuiSelectableFlags_Menu | ImGuiSelectableFlags_DontClosePopups |
    ...
    if (window->DC.LayoutType != ImGuiLayoutType_Horizontal) // (window->Flags &
                                                             // (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
            ...
            g.OpenPopupStack[g.CurrentPopupStack.Size].ParentWindow == window)
        {
            if (ImGuiWindow* next_window = g.OpenPopupStack[g.CurrentPopupStack.Size].Window)
            {
                ...
                tb.y = ta.y + ImMax((tb.y - extra) - ta.y, -100.0f); // triangle is maximum 200 high to limit the slope
    ...
    if (menu_is_open)
    {
        ...
        ImGuiWindowFlags flags = ImGuiWindowFlags_ShowBorders | ImGuiWindowFlags_AlwaysAutoResize |
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ...
    ColorButton(
        ...
        (flags & (ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf)) |
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    float off = -0.75f; // The border (using Col_FrameBg) tends to look off when color is near-opaque and rounding is
                        // enabled. This offset seemed like a good middleground to reduce those artefacts.
    bb_inner.Expand(off);
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col.w < 1.0f)
    {
        ...
        window->DrawList->AddRectFilled(bb_inner.Min, ImVec2(mid_x, bb_inner.Max.y), GetColorU32(col_without_alpha),
                                        rounding, ImGuiCorner_TopLeft | ImGuiCorner_BotLeft);
    }
    else
    {
        RenderColorRectWithAlphaCheckerboard(
            bb_inner.Min, bb_inner.Max, GetColorU32((flags & ImGuiColorEditFlags_AlphaPreview) ? col : col_without_alpha),
            grid_step, ImVec2(off, off), rounding);
    }
    if (window->Flags & ImGuiWindowFlags_ShowBorders)
        RenderFrameBorder(bb.Min, bb.Max, rounding);
    else
        window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_FrameBg), rounding); // Color button are often in
...
static void ColorPickerOptionsPopup(ImGuiColorEditFlags flags, float* ref_col)
{
    ...
    {
        ImVec2 picker_size(
            g.FontSize * 8, ImMax(g.FontSize * 8 - (SmallSquareSize() + g.Style.ItemInnerSpacing.x), 1.0f)); // FIXME:
                                                                                                             // Picker
                                                                                                             // size
                                                                                                             // copied
                                                                                                             // from
...
static void PaintVertsLinearGradientKeepAlpha(
    ImDrawVert* vert_start, ImDrawVert* vert_end, ImVec2 gradient_p0, ImVec2 gradient_p1, ImU32 col0, ImU32 col1)
{
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float sv_picker_size =
        ImMax(bars_width * 1,
              CalcItemWidth() - (alpha_bar ? 2 : 1) * (bars_width + style.ItemInnerSpacing.x)); // Saturation/Value
                                                                                                // picking
                                                                                                // box
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        for (int n = 0; n < 6; n++)
        {
            ...
            PaintVertsLinearGradientKeepAlpha(draw_list->_VtxWritePtr - (draw_list->_VtxCurrentIdx - vert_start_idx),
...
void ImGui::Separator()
{
    ...
    if ((flags & (ImGuiSeparatorFlags_Horizontal | ImGuiSeparatorFlags_Vertical)) == 0)
        flags |= (window->DC.LayoutType == ImGuiLayoutType_Horizontal) ? ImGuiSeparatorFlags_Vertical :
                                                                         ImGuiSeparatorFlags_Horizontal;
    IM_ASSERT(ImIsPowerOfTwo((int)(flags & (ImGuiSeparatorFlags_Horizontal | ImGuiSeparatorFlags_Vertical)))); // Check
                                                                                                               // that
                                                                                                               // only 1
                                                                                                               // option
                                                                                                               // is
...
void ImGui::EndGroup()
{
    ...
    if (group_data.AdvanceCursor)
    {
        window->DC.CurrentLineTextBaseOffset =
            ImMax(window->DC.PrevLineTextBaseOffset, group_data.BackupCurrentLineTextBaseOffset); // FIXME: Incorrect,
                                                                                                  // we should grab the
                                                                                                  // base offset from
                                                                                                  // the *first line* of
                                                                                                  // the group but it is
    ...
    // will be functional on the entire group. It would be be neater if we replaced window.DC.LastItemId by e.g. 'bool
    // LastItemIsActive', but if you search for LastItemId you'll notice it is only used in that context.
...
void ImGui::NewLine()
{
    ...
    window->DC.LayoutType = ImGuiLayoutType_Vertical;
    if (window->DC.CurrentLineHeight > 0.0f) // In the event that we are on a line with items that is smaller that
...
static float GetDraggedColumnOffset(int column_index)
{
    ...
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    IM_ASSERT(column_index > 0); // We cannot drag column 0. If you get this assert you may have a conflict between the
...
void ImGui::SetColumnOffset(int column_index, float offset)
{
    ...
    const bool preserve_width =
        !(window->DC.ColumnsFlags & ImGuiColumnsFlags_NoPreserveWidths) && (column_index < window->DC.ColumnsCount - 1);
    ...
    if (!(window->DC.ColumnsFlags & ImGuiColumnsFlags_NoForceWithinWindow))
        offset =
            ImMin(offset, window->DC.ColumnsMaxX - g.Style.ColumnsMinSpacing * (window->DC.ColumnsCount - column_index));
...
float ImGui::GetColumnWidth(int column_index)
{
    ...
    return OffsetNormToPixels(
        window, window->DC.ColumnsData[column_index + 1].OffsetNorm - window->DC.ColumnsData[column_index].OffsetNorm);
}
...
void ImGui::PushColumnClipRect(int column_index)
{
    ...
    PushClipRect(
        window->DC.ColumnsData[column_index].ClipRect.Min, window->DC.ColumnsData[column_index].ClipRect.Max, false);
}
...
void ImGui::BeginColumns(const char* id, int columns_count, ImGuiColumnsFlags flags)
{
    ...
    // window->DC.ColumnsMaxX = content_region_width - window->Scroll.x -((window->Flags & ImGuiWindowFlags_NoScrollbar)
    // ? 0 : g.Style.ScrollbarSize);// - window->WindowPadding().x;
    ...
    for (int column_index = 0; column_index < columns_count + 1; column_index++)
    {
        ...
        if (!(window->DC.ColumnsFlags & ImGuiColumnsFlags_NoForceWithinWindow))
            t = ImMin(
                t, PixelsToOffsetNorm(window, window->DC.ColumnsMaxX - g.Style.ColumnsMinSpacing *
...
void ImGui::EndColumns()
{
    ...
    window->DC.CursorMaxPos.x = ImMax(window->DC.ColumnsStartMaxPosX, window->DC.ColumnsMaxX); // Columns don't grow
                                                                                               // parent
        ...
        for (int i = 1; i < window->DC.ColumnsCount; i++)
        {
            ...
            if (!(window->DC.ColumnsFlags & ImGuiColumnsFlags_NoResize))
            {
                ...
                if (held && g.ActiveIdIsJustActivated)
                    g.ActiveIdClickOffset.x -= column_w; // Store from center of column line (we used a 8 wide rect for
                                                         // columns clicking). This is used by GetDraggedColumnOffset().
                if (held)
                    dragging_column = i;
            }
    ...
    if (ImGui::Begin("ImGui Metrics", p_open))
    {
        ...
        struct Funcs
        {
            static void NodeDrawList(ImDrawList* draw_list, const char* label)
            {
                ...
                {
                    ImGui::SameLine();
                    ImGui::TextColored(ImColor(255, 100, 100), "CURRENTLY APPENDING"); // Can't display stats for active
                ...
                ImDrawList* overlay_draw_list = &GImGui->OverlayDrawList; // Render additional visuals into the top-most
                                                                          // draw list
                     ...
                     elem_offset += pcmd->ElemCount, pcmd++)
                {
                    ...
                    bool pcmd_node_open = ImGui::TreeNode(
                        ...
                        draw_list->IdxBuffer.Size > 0 ? "indexed" : "non-indexed", pcmd->TextureId, pcmd->ClipRect.x,
                    ...
                    while (clipper.Step())
                             ...
                             prim < clipper.DisplayEnd; prim++)
                        {
                            ...
                            for (int n = 0; n < 3; n++, vtx_i++)
                            {
                                ...
                                buf_p +=
                                    sprintf(buf_p, "%s %04d { pos = (%8.2f,%8.2f), uv = (%.6f,%.6f), col = %08X }\n",
        ...
        if (ImGui::TreeNode("Basic state"))
        {
            ...
            ImGui::Text("HoveredId: 0x%08X/0x%08X", g.HoveredId, g.HoveredIdPreviousFrame); // Data is "in-flight" so
