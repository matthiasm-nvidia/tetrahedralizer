...
// compatibility! Implement maths operators for ImVec2 (disabled by default to not collide with using
...
// Helpers: UTF-8 <> wchar
IMGUI_API int ImTextStrToUtf8(char* buf, int buf_size, const ImWchar* in_text, const ImWchar* in_text_end); // return
                                                                                                            // output
                                                                                                            // UTF-8
                                                                                                            // bytes
                                                                                                            // count
IMGUI_API int ImTextCharFromUtf8(unsigned int* out_char, const char* in_text, const char* in_text_end); // return input
                                                                                                        // UTF-8 bytes
                                                                                                        // count
IMGUI_API int ImTextStrFromUtf8(ImWchar* buf,
                                ...
                                const char** in_remaining = NULL); // return
                                                                   // input
                                                                   // UTF-8
                                                                   // bytes
                                                                   // count
...
// Helpers: Misc
IMGUI_API ImU32 ImHash(const void* data, int data_size, ImU32 seed = 0); // Pass data_size==0 for zero-terminated
                                                                         // strings
...
enum ImGuiButtonFlags_
{
    ...
    ImGuiButtonFlags_PressedOnClickRelease = 1 << 1, // return true on click + release on same item [DEFAULT if no
    ...
    ImGuiButtonFlags_AllowOverlapMode = 1 << 10 // require previous frame HoveredId to either match id or be null before
                                                // being usable
};
...
struct ImGuiContext
{
    ...
    ImDrawList OverlayDrawList; // Optional software render of mouse cursors, if io.MouseDrawCursor is set + a few debug
                                // overlays
    ...
    ImGuiTextBuffer* LogClipboard; // Else log to clipboard. This is pointer so our GImGui static constructor doesn't
...
enum ImGuiItemFlags_
{
    ...
    ImGuiItemFlags_SelectableDontClosePopup = 1 << 4, // false    // MenuItem/Selectable() automatically closes current
                                                      // Popup window
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImDrawList* DrawList;
    ImGuiWindow* ParentWindow; // Immediate parent in the window stack *regardless* of whether this window is a child
                               // window or not)
    ImGuiWindow* RootWindow; // Generally point to ourself. If we are a child window, this is pointing to the first
                             // non-child parent window.
    ImGuiWindow* RootNonPopupWindow; // Generally point to ourself. Used to display TitleBgActive color and for
                                     // selecting which window to use for NavWindowing
...
IMGUI_API bool FocusableItemRegister(ImGuiWindow* window, ImGuiID id, bool tab_stop = true); // Return true if focus is
                                                                                             // requested
...
IMGUI_API void VerticalSeparator(); // Vertical separator, for menu bars (use current line height). not exposed because
                                    // it is misleading what it doesn't have an effect on regular layout.

// FIXME-WIP: New Columns API
IMGUI_API void BeginColumns(const char* id, int count, ImGuiColumnsFlags flags = 0); // setup number of columns. use an
...
IMGUI_API const char* FindRenderedTextEnd(const char* text, const char* text_end = NULL); // Find the optional ## from
                                                                                          // which we stop displaying
                                                                                          // text.
